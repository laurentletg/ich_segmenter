...
#
#
# #####Testing working zoom left, scrolling and move withotu error
# import vtk
# import slicer
#
...
#
    def zoom(self):
        ...
        factor = factor ** (abs(deltaY) / zoomSpeed)  # Adjust factor based on deltaY
...
#
# works well but non focuising zone
# import vtk
# import slicer
#
# class CustomInteractorStyle(vtk.vtkInteractorStyleImage):
...
#
    def zoom(self):
        ...
        factor = 1.01 if deltaY > 0 else 0.99
        zoomSpeed = 1  # Increase the zoom speed
        factor = factor ** (abs(deltaY) / zoomSpeed)  # Adjust factor based on deltaY
        sliceNode.SetFieldOfView(fov[0] * factor, fov[1] * factor, fov[2])
#
#     def getSliceSpacing(self):
...
#
    def zoom(self):
        ...
        imageCenter = [sliceNode.GetFieldOfView()[i] / 2 for i in range(3)]
        deltaX = currentPos[0] - imageCenter[0]
        deltaY = currentPos[1] - imageCenter[1]

        if deltaY > 0:
            slicer.modules.markups.logic().ZoomInSlice()
        else:
            slicer.modules.markups.logic().ZoomOutSlice()
#
#     def getSliceSpacing(self):
#         volumeNode = self.sliceLogic.GetBackgroundLayer().GetVolumeNode()
#         if volumeNode:
