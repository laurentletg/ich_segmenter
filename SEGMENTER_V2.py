...
class SemiAutoPheToolInstructionsWindow(qt.QWidget):
    def __init__(self, segmenter, parent=None):
        ...
        self.minimumLabel = qt.QLabel(
            "Click <b>Continue</b> and draw a generous boundary of the ICH and PHE complex. Note that the boundary may be drawn in multiple views. When you are finished drawing the boundary, click on <b>Show Result</b> in the main extension menu. "
...
class SEGMENTER_V2(ScriptedLoadableModule):
    ...
    def __init__(self, parent):
        ...
        self.parent.contributors = [
            "Delphine Pilon, An Ni Wu, Emmanuel Montagnon, Laurent Letourneau-Guillon"]  # TODO: replace with "Firstname Lastname (Organization)"
...
class SEGMENTER_V2Widget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def setup(self):
        ...
        self.ui.pushButton_ToggleVisibility.connect('clicked(bool)',
        ...
        self.ui.pushButton_SemiAutomaticPHE_Launch.connect('clicked(bool)',
                                                           self.onPushButton_SemiAutomaticPHE_Launch)
        self.ui.pushButton_SemiAutomaticPHE_ShowResult.connect('clicked(bool)',
                                                               self.onPushButton_SemiAutomaticPHE_ShowResult)
        ...
        self.ui.pushButton_check_errors_labels.connect('clicked(bool)',
        ...
        self.listichtype = [self.ui.ichtype_trauma, self.ui.ichtype_spontaneous,
                            self.ui.ichtype_infarct, self.ui.ichtype_metal_artifacts,
                            self.ui.ichtype_surgical_changes,
                            self.ui.ichtype_ventricular_drain, self.ui.ichtype_motion,
                            self.ui.ichtype_simple, self.ui.ichtype_complex]

        self.listichloc = [self.ui.ichloc_lob_frontal, self.ui.ichloc_lob_parietal,
                           self.ui.ichloc_lob_occipital, self.ui.ichloc_lob_temporal,
                           self.ui.ichloc_deep_bganglia,
                           self.ui.ichloc_deep_thalamus, self.ui.ichloc_deep_ic,
                           self.ui.ichloc_deep_ext_capsule, self.ui.ichloc_cerebellum_deep,
                           self.ui.ichloc_cerebellum_lobar, self.ui.ichloc_pons,
                           self.ui.ichloc_pons, self.ui.ichloc_brainstem]

        self.listEMs = [self.ui.EM_barras_density, self.ui.EM_barras_margins, self.ui.EM_black_hole,
                        self.ui.EM_blend,
                        self.ui.EM_fl_level, self.ui.EM_hypodensity, self.ui.EM_island,
                        self.ui.EM_satellite, self.ui.EM_swirl]

        self.flag_ICH_in_labels = False
        self.flag_PHE_in_labels = False
        for label in self.config_yaml["labels"]:
            if "ICH" in label["name"].upper() or "HEMORRHAGE" in label[
                "name"].upper() or "HÉMORRAGIE" in label["name"].upper() or "HEMORRAGIE" in label[
                "name"].upper() or "HAEMORRHAGE" in label["name"].upper():
                self.flag_ICH_in_labels = True
            if "PHE" in label["name"].upper() or "EDEMA" in label["name"].upper() or "OEDEME" in \
                    label["name"].upper() or "OEDÈME" in label["name"].upper():
    ...
    def getDefaultDir(self):
        self.DEFAULT_VOLUME_DIR = qt.QFileDialog.getExistingDirectory(None,
                                                                      "Open default directory",
                                                                      self.DEFAULT_VOLUME_DIR,
    ...
    def onPushButton_uint8casting(self):
        ...
        reply = qt.QMessageBox.question(None, 'Message',
                                        'Casiting to uint8. Do you want to overwrite the original segmentation files?',
    ...
    def cast_segmentation_to_uint8(self):
        ...
        for case in self.predictions_paths:
            ...
            if input_path.endswith('.nii') or input_path.endswith('.nii.gz'):
                ...
                raise ValueError(
                    'The input segmentation file must be in nii, nii.gz or nrrd format.')

    def onBrowseFoldersButton(self):
        # LLG get dialog window to ask for directory
        self.CurrentFolder = qt.QFileDialog.getExistingDirectory(None, "Open a folder",
                                                                 self.DEFAULT_VOLUME_DIR,
                                                                 qt.QFileDialog.ShowDirsOnly)
        ...
        print(f'regex pattern: {self.VOL_REGEX_PATTERN}')
        ...
        try:
            self.Cases = sorted([re.findall(self.VOL_REGEX_PATTERN, os.path.basename(i))[0] for i in
                                 self.CasesPaths])
    ...
    def createNewSegments(self):
        ...
        self.onPushButton_select_label(first_label_segment_name,
                                       self.config_yaml["labels"][0]["lower_bound_HU"],
                                       self.config_yaml["labels"][0]["upper_bound_HU"])
    ...
    def onNewLabelSegm(self, label_name, label_color_r, label_color_g, label_color_b, label_LB_HU,
                       label_UB_HU):
    ...
    def onPushButton_SemiAutomaticPHE_Launch(self):
        ...
        self.onPushButton_select_label(PHE_segment_name, PHE_label["lower_bound_HU"],
                                       PHE_label["upper_bound_HU"])
    ...
    def ClearPHESegment(self):
        ...
        self.onNewLabelSegm(PHE_label["name"], PHE_label["color_r"], PHE_label["color_g"],
                            PHE_label["color_b"], PHE_label["lower_bound_HU"],
                            PHE_label["upper_bound_HU"])
    ...
    def onSaveSegmentationButton(self):
        ...
        if self.annotator_name and self.time is not None:
            ...
            self.outputTimeFile = os.path.join(self.output_dir, 'annotation times',
                                               '{}_Case_{}_time_{}.csv'.format(self.annotator_name,
            ...
            self.outputSegmFile = os.path.join(self.output_dir, 'segmentations',
                                               "{}_{}_{}.seg.nrrd".format(
                                                   output_file_pt_id_instanceUid,
                                                   self.annotator_name, self.revision_step[0]))
        ...
        # If annotator_name empty or timer not started.
            ...
            if not self.annotator_name:
                ...
                msgboxtime2.setText(
                    "Error: timer is not started for some unknown reason. Restart and save again. File not saved!")
                msgboxtime2.exec()
    ...
    def onBrowseFolders_2Button(self):
        self.predictionFolder = qt.QFileDialog.getExistingDirectory(None, "Open a folder",
                                                                    self.DEFAULT_SEGMENTATION_DIR,
                                                                    qt.QFileDialog.ShowDirsOnly)

        self.predictions_paths = sorted(
            glob(os.path.join(self.predictionFolder, f'{self.SEGM_FILE_TYPE}')))
    ...
    def load_mask_v2(self):
        ...
        if not self.config_yaml['REVIEW_MODE']:
            ...
            msg_warning_delete_segm_node.setText(
                'This will delete the current segmentation. Do you want to continue?')
            msg_warning_delete_segm_node.setIcon(qt.QMessageBox.Warning)
            msg_warning_delete_segm_node.setStandardButtons(
        ...
        try:
            self.predictions_names = sorted(
                [re.findall(self.SEGM_REGEX_PATTERN, os.path.split(i)[-1]) for i in
        ...
        if self.currentPredictionPath != "":
            ...
            msg_no_such_case.setText(
                'There are no mask for this case in the directory that you chose!')
        ...
        if 'nii' in self.config_yaml['segmentation_extension']:
            ...
            if first_segment_name.startswith("Segment_"):
                ...
                for i in range(self.segmentationNode.GetSegmentation().GetNumberOfSegments()):
                    segment_name = self.segmentationNode.GetSegmentation().GetNthSegment(
                        i).GetName()
                    ...
                    for label in self.config_yaml["labels"]:
                        if label["value"] == int(segment_name.split("_")[-1]):
                            self.segmentationNode.GetSegmentation().GetNthSegment(i).SetName(
                                label['name'])
                            # set color
                            self.segmentationNode.GetSegmentation().GetNthSegment(i).SetColor(
                                label["color_r"] / 255,
                                label["color_g"] / 255,
                                label["color_b"] / 255)
    ...
    def add_missing_nifti_segment(self):
        if 'nii' in self.config_yaml['segmentation_extension']:
            for label in self.config_yaml['labels']:
                name = label['name']
                segment_names = [
                    self.segmentationNode.GetSegmentation().GetNthSegment(node).GetName() for node
                    in
    ...
    def update_current_case_paths_by_segmented_volumes(self):
        ...
        segmented_IDs = [re.findall(self.SEGM_REGEX_PATTERN, os.path.basename(segmentation))[0] for
                         segmentation in
                         segmentations]
    ...
    def onDropDownButton_label_select(self, value):
        ...
        try:
            ...
            self.onPushButton_select_label(segment_name, label["lower_bound_HU"],
    ...
    def check_match_label_name_value(self):
        ...
        print(
            f'Writing a copy of the slicerio corrected segmentation file  {self.outputSegmFile} with the corrected labels and names')
        output_file_pt_id_instanceUid = re.findall(self.VOL_REGEX_PATTERN_PT_ID_INSTUID_SAVE,
                                                   os.path.basename(self.currentCasePath))[0]
        ...
        output_path = os.path.join(output_dir_segmentation_file_corrected,
                                   f'Slicerio_corrected_segmentation_{output_file_pt_id_instanceUid}.seg.nrrd')
        ...
        try:
            ...
            print(e)
            print('Segmentation name {} to label value {}'.format(extracted_header['Segment0_Name'],
                                                                  extracted_header[
                                                                      'Segment0_LabelValue']))
            header['Segment0_LabelValue'] = 1
            header['Segment0_Name'] = ich_name
            print('Segmentation name {} to label value {}'.format(extracted_header['Segment1_Name'],
                                                                  extracted_header[
                                                                      'Segment1_LabelValue']))
            header['Segment1_LabelValue'] = 2
            header['Segment1_Name'] = ivh_name
            print('Segmentation name {} to label value {}'.format(extracted_header['Segment2_Name'],
                                                                  extracted_header[
                                                                      'Segment2_LabelValue']))
    ...
    def check_for_outlier_labels(self):
        ...
        for segmentID in segmentIDs:
            ...
            array_range = labelArray[(volumeArray < self.OUTLIER_THRESHOLD_LB) | (
    ...
    def save_statistics(self):
        ...
        segStatLogic.getParameterNode().SetParameter(
            "LabelSegmentStatisticsPlugin.obb_direction_ras_x_.enabled", str(True))
        segStatLogic.getParameterNode().SetParameter(
            "LabelSegmentStatisticsPlugin.obb_direction_ras_y_.enabled", str(True))
        segStatLogic.getParameterNode().SetParameter(
            "LabelSegmentStatisticsPlugin.obb_direction_ras_z_.enabled", str(True))
        segStatLogic.getParameterNode().SetParameter(
            "LabelSegmentStatisticsPLugin.obb_diameter_mm.enables", str(True))
        ...
        outputFilename = 'Volumes_{}_{}_{}.{}'.format(output_file_pt_id_instanceUid,
                                                      self.annotator_name, self.revision_step[0],
                                                      'csv')
    ...
    def get_line_measure(self):
        ...
        if markupsLogic:
            ...
            output_file_pt_id_instanceUid = re.findall(self.VOL_REGEX_PATTERN_PT_ID_INSTUID_SAVE,
                                                       os.path.basename(self.currentCasePath))[0]
            outputFilename = 'Line_measure_{}_{}_{}.{}'.format(output_file_pt_id_instanceUid,
                                                               annotator_name, revision_step, 'csv')
            outputFilename = os.path.join(output_dir, outputFilename)
            df = pd.DataFrame({'id': output_file_pt_id_instanceUid, 'annotator': annotator_name,
                               'revision_step': revision_step, 'length': [length]})
            df.to_csv(outputFilename, index=False)
            ...
            output_screen_capture_filename = os.path.join(output_dir,
                                                          f'line_measure_{output_file_pt_id_instanceUid}_{annotator_name}_{revision_step}.png')
    ...
    def get_screenshot(self):
        view = slicer.app.layoutManager().sliceWidget('Red').sliceView()
        cap = ScreenCapture.ScreenCaptureLogic()
        output_screen_capture_filename = os.path.join(self.output_dir,
                                                      f'screenshot_{self.currentCase}.png')
        cap.captureImageFromView(view, output_screen_capture_filename)
        comments = self.ui.textEdit_screenshot.toPlainText()
        output_dir = os.path.joint(self.output_dir, 'screenshots')
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        with open(os.path.join(self.output_dir, f'screenshot_comments_{self.currentCase}.txt'),
                  'w') as f:
            f.write(comments)
